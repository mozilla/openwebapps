    ...
    for (i = 0; i < 7; i++) {
      if (arrangedPage[i]) {
        if (gAppItemCache[arrangedPage[i]].position().left != (appBoxWidth * i)) {
          ...
          gAppItemCache[arrangedPage[i]].animate({
            left: (appBoxWidth * i)
          }, 100);
        }
      }
...
function redrawPage(page, animated) {
  for (var i = 0; i < 6; i++) {
    if (gDashboardState.pages[page][i]) {
      if (animated) {
        gAppItemCache[gDashboardState.pages[page][i]].css({
          left: (appBoxWidth * i)
        });
      } else {
        gAppItemCache[gDashboardState.pages[page][i]].animate({
          left: (appBoxWidth * i)
        }, 100);
      }
    }
...
function _onMouseUp(e) {
  ...
  if (_draggedApp) {
    ...
    gAppItemCache[_draggedApp].animate({
      left: (_draggedAppLastSlot * appBoxWidth)
    }, 100);
...
function updateState(newState) {
  ...
  if (gDashboardState.pages == undefined) {
    ...
    var a = 0;
    for (origin in gApps) {
      if (gDashboardState.pages[Math.floor(a / 6)] == undefined) {
        gDashboardState.pages[Math.floor(a / 6)] = [];
      }
...
function addEmptyPageToDash() {
  ...
  //grow the dashboard
  $('#dashboard').css({
    width: ((numPages + 1) * (screenWidth + 2)),
    height: screenHeight
  });
