...
const DEFAULT_SNIPPETS_URLS = ["http://www.mozilla.com/firefox/4.0/features", "https://addons.mozilla.org/firefox/?src=snippet"];
...
function setupSearchEngine() {
  ...
  if (gSearchEngine.params) gSearchEngine.searchUrl += "&" + gSearchEngine.params;
...
function loadSnippets() {
  ...
  if (updateURL && (!lastUpdate || Date.now() - lastUpdate > SNIPPETS_UPDATE_INTERVAL_MS)) {
...
function activateSnippetsButtonClick(aElt) {
  ...
  if (links.length == 1) {
    document.getElementById("snippets").addEventListener("click", function(aEvent) {
      if (aEvent.target.nodeName != "a") window.location = links[0].href;
    }, false);
  }
}
...
function displayPlaceholder(event) {
  ...
  for (var i = 0; i < dockIcons.length; i++) {
    ...
    if ($(dockIcons[i]).hasClass("appInDock")) {
      $("#dock").append(dockIcons[i])
    };
  }
  resizeDashboard();
}
...
function updateWidgets() {
      ...
      if (gApps[app].manifest && gApps[app].manifest.widget) {
        ...
        if (existingWidget[0]) {
          ...
          if (!gDashboardState.widgetPositions[gApps[app].origin32]) {
            ...
            gDashboardState.widgetPositions[gApps[app].origin32] = {
              "top": 0,
              "left": 0,
              "height": wH,
              "width": wW,
  ...
  $(".widgetFrame").each(function() {
    ...
    if (!app) {
      ...
      $(this).children(selectorString).animate({
        "height": wPos.height,
        "width": wPos.width
      });
    };
...
function createWidget(install, top, left, height, width, zIndex) {
  ...
  widgetFrame.draggable({
    containment: widgetSpace,
    zIndex: 1000,
    stack: ".widgetFrame",
    ...
    stop: function(event, ui) {
      ...
      gDashboardState.widgetPositions[install.origin32] = {
        "top": ui.helper.position().top,
        "left": ui.helper.position().left,
        "height": ui.helper.height() - 16,
        "width": ui.helper.width() - 16,
        "zIndex": ui.helper.zIndex()
      };
  ...
  widgetFrame.resizable({
    containment: widgetSpace,
    handles: 'se',
    alsoResize: selectorString,
    minHeight: 64,
    minWidth: 64,
    maxHeight: 800,
    maxWidth: 800,
...
function createAppInfoPane(origin32) {
  ...
  if (install.manifest.widget) {
    ...
    widgetButton.click(function() {
      if (toggleWidgetVisibility(install.origin32)) {
        widgetButton.addClass("glowy-green-text");
      } else {
    ...
    widgetButton.mouseenter(function() {
    ...
    widgetButton.mouseleave(function() {
      widgetButton.animate({
        "font-size": 14 + "px",
        "padding-top": "8px",
        "padding-bottom": "0px"
