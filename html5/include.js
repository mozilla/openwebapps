    ...
    var Channel = (function() {
      ...
      function s_addBoundChan(origin, scope, handler) {
        ...
        if (origin === '*') {
          ...
          // we must check only '*'
          if ((s_boundChans['*'] && s_boundChans['*'][scope]) || (s_boundChans[origin] && s_boundChans[origin][scope])) {
            exists = true;
          }
        }
        ...
        build: function(cfg) {
          var debug = function(m) {
            if (cfg.debugOutput && window.console && window.console.log) {
              // try to stringify, if it doesn't work we'll let javascript's built in
              // toString do its magic
              try {
