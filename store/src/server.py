...
class VerifyHandler(WebHandler):
  def get(self, appID):
    ...
    try:
      app = model.application(appID)
      parsedApp = json.loads(app.manifest)
    except:
    ...
    if app.price == 0 and isIPhone:
      self.render("iphone_verify.html", validationURL=app.baseURL + parsedApp["launch_path"], appName=app.name, appIcon=app.icon96URL, appLaunchScreen=app.icon96URL)
      return
    ...
    if model.purchase_for_user_app(uid, appID):
      ...
      verifyURL = "%s%s?%s" % (app.baseURL, parsedApp["launch_path"], urllib.urlencode( { 
        "moz_store.status":"ok",
      ...
      self.redirect("%s%s?%s" % (app.baseURL, parsedApp["launch_path"], urllib.urlencode( { 
...
TaskTrackerApp = {"name":"Task Tracker", 
                ...
                "base_url": "https://tasktracker.mozillalabs.com/",
                "app_urls": ["http://tasktracker.mozillalabs.com/",
...
GmailApp = {"name":"Gmail",
                "description":"7+ GB of storage, less spam, and mobile access. Gmail is email that's intuitive, efficient, and useful. And maybe even fun.",
                  ...
                  "base_url": "http://mail.google.com/mail/",
                  "launch_path": "",
                "developer": {
                  "name":"Google",
                  "url":"http://google.com"
